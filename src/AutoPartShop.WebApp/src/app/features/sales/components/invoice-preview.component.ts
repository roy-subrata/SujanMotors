...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, DialogModule, ButtonModule, TooltipModule, DividerModule, TranslatePipe],
...
export class InvoicePreviewComponent implements OnInit, OnChanges {
    ...
    /** Cash change to return to the customer when tendered exceeded the grand total. */
    get changeDue(): number {
        const d = this.invoiceData;
        if (!d) return 0;
        if (d.changeDue != null) return d.changeDue;
        return d.paidAmount > d.grandTotal ? d.paidAmount - d.grandTotal : 0;
    }
    ...
    async downloadPdf(): Promise<void> {
        ...
        try {
            await new Promise<void>((resolve, reject) => this.pdfService.downloadServerPdf(this.invoiceId!, this.invoiceData!.invoiceNumber).subscribe({ next: () => resolve(), error: reject }));
