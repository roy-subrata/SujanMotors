...
export class SalesReturnsListComponent implements OnInit {
            ...
            error: (err) => {
                this.salesReturnService.getAllSalesReturns().subscribe({
                    next: (returns) => {
                        ...
                        if (this.searchTerm) {
                            filteredData = filteredData.filter((r) => r.returnNumber?.toLowerCase().includes(this.searchTerm.toLowerCase()) || r.salesOrderId?.toLowerCase().includes(this.searchTerm.toLowerCase()));
                        }
                        if (this.filterStatus) {
    ...
    private exportToCSV(data: SalesReturnResponse[]): void {
        ...
        const csvData = data.map((ret) => [ret.returnNumber, ret.salesOrderId, this.formatDate(ret.createdAt), ret.reason, ret.totalRefundAmount.toString(), ret.status]);
