...
export class PurchaseReturnsComponent implements OnInit {
    ...
    private exportToCSV(): void {
        ...
        const rows = this.purchaseReturns.map((pr) => [pr.returnNumber, pr.supplierName || '', pr.returnDate ? new Date(pr.returnDate).toLocaleDateString() : '', pr.status, pr.refundAmount.toString()]);
